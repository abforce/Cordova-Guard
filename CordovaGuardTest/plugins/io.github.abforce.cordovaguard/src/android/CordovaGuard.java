...
public class CordovaGuard extends CordovaPlugin {
    ...
    private static final String LOG_TAG = "CordovaGuardPlugin";

    private List<Permission> permissions;

    // Used when instantiated via reflection by PluginManager
    public CordovaGuardPlugin() {
    }

    // These can be used by embedders to allow Java-configuration of whitelists.
    public CordovaGuardPlugin(Context context) {
        this(new ArrayList<Permission>());
        new CustomConfigXmlParser().parse(context);
    }

    public CordovaGuardPlugin(XmlPullParser xmlParser) {
        this(new ArrayList<Permission>());
        new CustomConfigXmlParser().parse(xmlParser);
    }

    public CordovaGuardPlugin(List<Permission> permissions) {
        if (permissions == null) {
